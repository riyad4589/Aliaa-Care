...
const ProductDetail = () => {
  ...
  const handleAddToCart = () => {
    ...
    toast({ title: "Ajouté au panier", description: `${quantity} × ${product.name} ajouté à votre panier.` });
    setQuantity(1);
  };
    ...
    <Layout>
      ...
      <div className="container-full py-6 border-b border-border">
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <Link to="/products" className="hover:text-foreground transition-colors">Boutique</Link>
        ...
        <div className="container-full">
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-20">
            ...
            <div className="lg:col-span-7 space-y-4">
              <div className="relative aspect-[4/5] overflow-hidden bg-muted/30 group cursor-zoom-in rounded-lg">
                ...
                {product.images.length > 1 && (
                  <>
                    <button onClick={prevImage} className="absolute left-5 top-1/2 -translate-y-1/2 p-3 bg-background/90 backdrop-blur-md hover:bg-background transition-all duration-300 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 rounded-full">
                      <ChevronLeft className="w-5 h-5" />
                    </button>
                    <button onClick={nextImage} className="absolute right-5 top-1/2 -translate-y-1/2 p-3 bg-background/90 backdrop-blur-md hover:bg-background transition-all duration-300 opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 rounded-full">
                ...
                <div className="absolute top-5 left-5 flex flex-col gap-2">
                  {product.new && (
                    <span className="px-3 py-1.5 text-[10px] font-semibold tracking-[0.2em] uppercase bg-foreground text-background rounded-sm">Nouveau</span>
                  )}
                </div>
              </div>
            ...
            >
              ...
              <div className="mb-6">
                <span className="text-[11px] font-semibold tracking-[0.2em] uppercase text-muted-foreground block mb-3">Quantité</span>
              ...
              <div className="flex flex-col gap-3">
                ...
                <Button variant="outline" size="lg" className="rounded-none w-full py-6 text-sm tracking-[0.1em] uppercase" onClick={handleWishlistToggle}>
                  ...
                  {inWishlist ? "Sauvegardé" : "Ajouter aux Favoris"}
                </Button>
              </div>
        ...
        <section className="py-20 md:py-28 bg-linen">
          <div className="container-full">
            <div className="flex items-end justify-between mb-12">
              <div>
                <p className="text-[11px] font-semibold tracking-[0.3em] uppercase text-primary mb-3">Vous Aimerez Aussi</p>
                <h2 className="font-serif text-3xl md:text-4xl text-foreground">Plus de {collection?.name}</h2>
              </div>
              <Link to={`/products?collection=${collection?.slug}`}
